...
const PurchaseButtons = ({ index, stock, productId, fetchProductData }) => {
  ...
  const notify = () => toast("Only logged in users can buy !");
  console.log(user);
  ...
  return (
    <>
      <div
        className={style.purshaseButtons}
        style={{ display: stock !== 0 ? "flex" : "none" }}
      >
        <button className={style.counter}>
          <span className={style.decrease} onClick={decreaseOne}>
            -
          </span>
          <span className={style.count}>{count}</span>
          <span className={style.increase} onClick={increaseOne}>
            +
          </span>
        </button>
        <button
      ...
      <button
        className={style.soldOut}
        style={{ display: stock === 0 ? "block" : "none" }}
      >
        Sold Out
      </button>
    </>
